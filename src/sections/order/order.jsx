...
export default function OrderView() {
    ...
    const [orderSlicedData, setorderSlicedData] = useState("")
    ...
    useEffect(() => {
        if (allOrder && allOrder.length > 5) {
            const slicedData = allOrder.slice(0, PageLimit);
            setorderSlicedData(slicedData);
        }
    }, [allOrder, PageLimit]);

    const fetchMoreData = () => {
        const nextPageLimit = PageLimit + 4;
        setPageLimit(nextPageLimit);

        if (nextPageLimit < allOrder.length) {
            setTimeout(() => {
                const newSlicedData = allOrder.slice(0, nextPageLimit);
                setorderSlicedData(newSlicedData);
            }, 500);
        }
    };
    //infinite scrolling
    ...
    return (
        <Container className='mt-8' maxWidth="xl" >
            ...
            {allOrder && Number(allOrder?.length > 6) ?
                <>
                    <InfiniteScroll
                        dataLength={orderSlicedData.length}
                        next={fetchMoreData}
                        hasMore={allOrder.length > orderSlicedData.length ? true : false}
                        loader={<div className='flex justify-center items-center  h-80 w-[100%]'>
                            <Bars
                                visible={true}
                                height="80"
                                width="80"
                    ...
                    >
                        <div style={{ minWidth: "400px", overflowX: "auto" }}>
                            <CTable className='table table-bordered border-dark'>
                                <CTableHead>
                                ...
                                <CTableBody>
                                    {
                                        orderSlicedData?.length > 0 && orderSlicedData?.map((ele) => {
                                            const viewOrderItem = orderSlicedData && orderSlicedData?.filter((item) => item._id.toString() === orderItemID && orderItemID.toString())
                                            return (
                                                <CTableRow key={ele._id} striped color={ele.orderStatus === "CANCELLED" ? "danger" : ele?.orderStatus === "Delivered" ? 'success' : ele?.orderStatus === "Placed" ? 'warning' : 'light'}>
                ...
                <div style={{ minWidth: "400px", overflowX: "auto" }}>
                    <CTable className='table table-bordered border-dark'>
                        ...
                        <CTableBody>
                            {
                                allOrder?.length > 0 && allOrder?.map((ele) => {
